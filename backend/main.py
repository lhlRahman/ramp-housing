...
async def api_start_outreach(request: Request, body: StartOutreachRequest) -> dict[str, Any]:
    ...
    for r in results:
        if isinstance(r, Exception):
            ...
            outreach_results.append({"error": "Outreach dispatch failed"})
        else:
...
async def twilio_sms_webhook(request: Request):
    ...
    async with _outreach_locks[oid]:
        ...
        try:
            ...
            profile = get_renter_profile(row["renter_phone"])
            listing = outreach.get("listing") or {} if outreach else {}
...
async def api_outreach_send_sms(request: Request, outreach_id: str, body: ManualSMSRequest) -> dict[str, Any]:
    ...
    try:
        ...
        log.error("Manual SMS to %s failed: %s", landlord_phone, exc)
        raise HTTPException(status_code=400, detail=f"Failed to send SMS: {exc}")
